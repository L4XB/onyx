...
// ancestors listed before any snapshot can be probed.
func TestLocateBaseSnapshotInShallowClone(t *testing.T) {
	...
	match, err := LocateBaseSnapshot(tipSHA, GitCommitHistory{}, store, DefaultBaseWalkLimit)
	if err != nil {
		t.Fatalf("LocateBaseSnapshot: %v", err)
	}
	if match.Distance != 2 {
