...
type S3SnapshotStore struct {
	...
	Module string
	// FetchObject downloads s3url to destPath. Any error means the object is
	// unavailable: a missing object and a refused one look the same without
	// credentials.
...
// Fetch downloads the snapshot of a commit. A download failure is
// ErrSnapshotUnavailable, but a downloaded file that does not parse, or names
// another commit or module, is a hard error: the bucket is then wrong and a
// comparison against it would be misleading.
