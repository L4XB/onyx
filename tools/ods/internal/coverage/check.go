...
func Compare(profile *Profile, reference *Reference, tolerance float64) *Report {
	...
	if reference == nil {
		...
		report.Total = compareOne("total", profile.Total(), reference.Total, true, tolerance)
	}
	...
	for _, pkg := range profile.Packages {
		...
		report.Packages = append(report.Packages, compareOne(pkg.Package, pkg.Percent(), value, hasValue, tolerance))
	}
...
func compareOne(name string, percent, reference float64, hasReference bool, tolerance float64) Result {
