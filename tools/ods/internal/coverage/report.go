...
// WriteReport renders a report as an aligned table. Packages with no reference
// value yet show a blank column rather than a misleading zero.
func WriteReport(w io.Writer, report *Report) error {
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)

	if _, err := fmt.Fprintf(tw, "PACKAGE\tCOVERAGE\t%s\t\n", strings.ToUpper(referenceColumn(report))); err != nil {
		return err
	}
