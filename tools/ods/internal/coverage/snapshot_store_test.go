...
// A missing object and a refused one look the same without credentials, so
// both are "unavailable" and the caller falls back to the floors.
func TestS3SnapshotStore_fetchFailureIsUnavailable(t *testing.T) {
