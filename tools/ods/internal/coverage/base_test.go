...
// resolves, so the ancestors must be fetched first.
func TestLocateBaseSnapshot_shallowCloneFetches(t *testing.T) {
	sha := commitSha(0)
	history := &fakeCommitHistory{
		shallow:   true,
		resolved:  map[string]string{sha: sha},
	...
	if _, err := LocateBaseSnapshot(sha, history, store, DefaultBaseWalkLimit); err != nil {
		t.Fatalf("failed to locate the base snapshot: %v", err)
	}
